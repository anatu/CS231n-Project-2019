...
class ResNet101(object):
    ...
    def __init__(self, pretrained_model, device, num_classes=7, lr=0.0001, reg=0.0, dtype=np.float32, mode="ft_extract"):
        '''
        Initialize a new network based on the ResNet101 architecture
        '''
        ...
        self.device = device
        self.save_model_path = "./model_weights/best_model_weights_resnet.pt"
        ...
        self.model.fc = nn.Linear(num_features, num_classes).to(self.device)
    ...
    def set_parameter_requires_grad(self, mode):
        ...
        If "finetune_last", only finetunes the layers 7-10 of the network, keeping the first 6 frozen.
        ...
        if mode == "ft_extract":
            ...
            for layer in list(self.model.children())[:6]:
    ...
    def visualize_model(self, dataloaders, num_images=16):
        ...
        images_so_far = 0
        file_path_base = "./visuals/resnet_visuals/"
...
if __name__ == "__main__":
    ...
    # Set the device and load the pretrained model
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    resnet101 = models.resnet101(pretrained=True).to(device)

    # Initialize the model
    resnet_model = ResNet101(resnet101, device, num_classes=7, mode="finetune_last")
    
    # Train the last layer of the model
    resnet_model.train(dataloaders, dataset_sizes, num_epochs=20)
    
    # Or load from saved weights from previously retrained model
    ## resnet_model.load_model("./model_weights/best_model_weights_resnet.pt", train_mode = False)
    
    # Some visualizations
    ## resnet_model.visualize_model(dataloaders, num_images=25)

    # Can use eval_model method to evaluate the model after training
    ## resnet_model.eval_model(dataloaders, mode = 'val')
